    ...
    <main className="synora-shell">
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          <div className="status"><span className="dot" /> NETWORK {errors > 5 ? 'CRITICAL' : errors > 0 ? 'DEGRADED' : 'STABLE'}</div>
          <LogoutButton />
        </div>
      </header>

      <SynoraIntro completed={completed} />
      <ReturnMemory completed={completed} />
      ...
      <section className="dashboard">
        ...
        <div className="hero-core" aria-label="Core della rete SYNORA">
          <div className="core-orbit orbit-a" />
          <div className="core-orbit orbit-b" />
          <div className="core-orbit orbit-c" />
          <div className="core-sphere"><span>SYNORA</span><b>{String(completed).padStart(2, '0')}</b><small>NODES ONLINE</small></div>
          <div className="core-signal signal-1">PACKETS <b>STABLE</b></div>
          <div className="core-signal signal-2">LATENCY <b>17ms</b></div>
          <div className="core-signal signal-3">TRACE <b>ACTIVE</b></div>
        </div>

        <NetworkWeather completed={completed} errors={errors} hints={hints} />
        <SynoraConsole completed={completed} errors={errors} hints={hints} />
        <NexusPresence roomId={current} />
        <RadioTransmission roomId={current} />

        <div className="command-grid">
          <section className="district-terminal">
            <div className="section-kicker">CITY NETWORK // FIELD MAP</div>
            <div className="terminal-title">
              <div><span className="eyebrow">THE EIGHT DISTRICTS</span><h2>Riaccendi la città.</h2></div>
              <div className="node-counter"><b>{completed}</b><span>/ 08 ONLINE</span></div>
            </div>
        ...
        <PacketVision roomId={current} />
        <BlackoutMode roomId={current} />
        <div className="experience-grid">
          <IncidentLog roomId={current} errors={errors} />
          <HiddenClues roomId={current} />
        </div>
        <MetaStory completed={completed} />
        <SynoraArtifactArchive completed={completed} />
        <SynoraFinale completed={completed} errors={errors} hints={hints} />
        <TechnicianSignature completed={completed} errors={errors} hints={hints} />
      </section>
    </main>
  )
}
