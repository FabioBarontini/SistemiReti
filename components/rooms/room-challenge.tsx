...
export default function RoomChallenge({roomId}:{roomId:number}){
 ...
 return <div className="challenge-wrap">
   ...
   {roomId===1 && <>
    <Panel title="TRACE / FRAME 0017"><p className="instruction">Quattro frammenti sono stati recuperati. Attribuiteli al livello corretto e poi ricostruite il percorso inverso di rimozione degli header.</p><div className="packet-grid">{[['A','PORTA DESTINAZIONE','443'],['B','MAC SORGENTE','AA:7B:19:CC:04:91'],['C','IP DESTINAZIONE','172.16.40.20'],['D','PROTOCOLLO','HTTPS']].map(([id,k,v])=><div className="packet-chip" key={id}><b>{id}</b><span>{k}</span><code>{v}</code><Input value={a[id]||''} onChange={v=>set(id,v)} placeholder="livello TCP/IP…"/></div>)}</div><div className="mini-question"><span>Rimozione degli header, dal frame ricevuto fino all'applicazione</span><Input value={a.order||''} onChange={v=>set('order',v)} placeholder="livello → livello → livello → livello"/></div></Panel>
    <Panel title="PDU / CONTROLLO DI COERENZA"><p className="instruction">Non basta riconoscere i livelli: indicate anche quale PDU attraversa ciascun passaggio.</p><div className="answer-pair three"><label>Trasporto</label><Input value={a.pduT||''} onChange={v=>set('pduT',v)} placeholder="PDU"/><label>Rete</label><Input value={a.pduR||''} onChange={v=>set('pduR',v)} placeholder="PDU"/></div></Panel>
   </>}
   ...
   {roomId===4 && <>
    ...
    <Panel title="DIAGNOSTICA ICMP"><p className="instruction">Il quarto hop non risponde. Interpretate la traccia e calcolate il TTL residuo al terzo router partendo da TTL=4.</p><div className="answer-pair"><label>Evento osservato</label><Input value={a.event||''} onChange={v=>set('event',v)} placeholder="descrivete cosa significa * * *"/><label>TTL al terzo hop</label><Input value={a.ttl||''} onChange={v=>set('ttl',v)} placeholder="numero"/></div></Panel>
   </>}
   ...
   {roomId===5 && <>
    ...
    <Panel title="3 / VERIFICA POST-REPAIR"><p className="instruction">Quando il terminale ha completato la procedura, dichiarate l'esito finale della route.</p><div className="answer-pair"><label>Route ripristinata</label><Input value={a.route||''} onChange={v=>set('route',v)} placeholder="rete /24"/><label>Perché?</label><Choice active={a.reason==='next-hop-raggiungibile'} onClick={()=>set('reason','next-hop-raggiungibile')}>il next-hop torna raggiungibile attraverso Gi0/2</Choice><label>Stato Gi0/2 dopo la riparazione</label><Input value={a.state||''} onChange={v=>set('state',v)} placeholder="status / line protocol"/></div></Panel>
   </>}
